...
AC_ARG_WITH(cblas-lib, [  --with-cblas-lib=PATH      Path to libcblas], [cblas_lib=$withval], [cblas_lib="/usr/local/atlas"])
AC_ARG_WITH(cblas-header, [  --with-cblas-header=PATH   Path to cblas.h],  [cblas_header=$withval], [cblas_header="/usr/include"])

if test "$cblas_lib" == "no"; then
AC_MSG_ERROR([cblas lib not found !!!])
fi

if test "$cblas_header" == "no"; then
AC_MSG_ERROR([cblas header not found !!!])
fi

AC_SUBST([cblas_lib])
AC_SUBST([cblas_header])

OLD_CPPFLAGS=$CPPFLAGS
CPPFLAGS="-I$cblas_header"

AC_CHECK_HEADER(cblas.h, [], [AC_MSG_ERROR([Couldn't find cblas.h])])

CPPFLAGS=$OLD_CPPFLAGS

CBLAS_LIBS="-L$cblas_lib -lcblas"
