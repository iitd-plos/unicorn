...
AC_ARG_WITH(mpi, [  --with-mpi=PREFIX      Prefix of your MPI installation], [mpi_prefix=$withval], [mpi_prefix="/usr/local/mpi"])
...
AC_LANG_CASE([C], [
	...
	AC_CHECK_PROGS(MPICC, mpicc hcc mpxlc_r mpxlc mpcc cmpicc, $CC, [path = $mpi_prefix/bin])
	ax_mpi_save_CC="$CC"
...
[C++], [
	...
	AC_CHECK_PROGS(MPICXX, mpic++ mpicxx mpiCC hcp mpxlC_r mpxlC mpCC cmpic++, $CXX, [path = $mpi_prefix/bin])
	ax_mpi_save_CXX="$CXX"
