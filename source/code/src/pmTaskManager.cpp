...
pmRemoteTask* pmTaskManager::CreateRemoteTask(pmCommunicatorCommand::remoteTaskAssignPacked* pRemoteTaskData)
{
	...
	START_DESTROY_ON_EXCEPTION(lDestructionBlock)
		...
		DESTROY_PTR_ON_EXCEPTION(lDestructionBlock, lInputMem, pmMemSection, (pRemoteTaskData->taskStruct.inputMemLength == 0) ? NULL : (pmMemSection::CheckAndCreateMemSection(pRemoteTaskData->taskStruct.inputMemLength, lOriginatingHost, pRemoteTaskData->taskStruct.inputMemGenerationNumber, true)));
		DESTROY_PTR_ON_EXCEPTION(lDestructionBlock, lOutputMem, pmMemSection, (pRemoteTaskData->taskStruct.outputMemLength == 0) ? NULL : (pmMemSection::CheckAndCreateMemSection(pRemoteTaskData->taskStruct.outputMemLength, lOriginatingHost, pRemoteTaskData->taskStruct.outputMemGenerationNumber, false)));
