            ...
            && !(pRequestingDevice->GetMachine() == PM_LOCAL_MACHINE && pRequestingDevice->GetType() == GetType()))
    {
        ...
        if((lAllotteeIter == lSecondaryAllotteeMap.end()) || (lAllotteeIter->second.size() < MAX_SUBTASK_MULTI_ASSIGN_COUNT - 1))
        {
            ...
            if(lShouldMultiAssign)
            {
                ...
                lShouldMultiAssign = (lLocalRateZero ? (lElapsedTime >= (1.0 / pRequestingDeviceExecutionRate) * lFactor) : (lExpectedRemoteTimeToExecute * SUBTASK_TRANSFER_OVERHEAD < lExpectedLocalTimeToFinish));
...
pmStubCUDA::pmStubCUDA(size_t pDeviceIndex, uint pDeviceIndexOnMachine)
    ...
    , mCudaCacheAllocationLength(0)
    , mCudaCacheEvictionLength(0)
    , mCudaCacheAllocationCount(0)
    , mCudaCacheEvictionCount(0)
#endif
{
}
...
void pmStubCUDA::CopyDataToPinnedBuffers(pmTask* pTask, ulong pSubtaskId, pmSplitInfo* pSplitInfo, const pmSubtaskInfo& pSubtaskInfo)
{
        ...
        if(lVector[pAddressSpaceIndex].requiresLoad)
        {
            ...
            if(lVisibilityType == SUBSCRIPTION_NATURAL)
            {
                ...
                if(lSubscriptionInfo.length)
                {
                    for(lIter = lBegin; lIter != lEnd; ++lIter)
                    {
                        ...
                        PMLIB_MEMCPY(lPinnedPtr, lDataPtr, lIter->second.first, std::string("pmStubCUDA::CopyDataToPinnedBuffers1"));
                    }
                }
            }
            else    // SUBSCRIPTION_COMPACT
            {
                ...
                if(lCompactViewData.subscriptionInfo.length)
                {
                    ...
                    if(lShadowMemAddr && pTask->IsReadOnly(pAddressSpace))
                    {
                        PMLIB_MEMCPY(lVector[pAddressSpaceIndex].pinnedPtr, lShadowMemAddr, lCompactViewData.subscriptionInfo.length, std::string("pmStubCUDA::CopyDataToPinnedBuffers2"));
                    }
                    else
                    {
                        size_t lBaseAddr = reinterpret_cast<size_t>(pAddressSpace->GetMem());
                        ...
                        for(lIter = lBegin; lIter != lEnd; ++lIter, ++lOffsetsIter)
                        {
                            ...
                            PMLIB_MEMCPY(lPinnedPtr, lDataPtr, lIter->second.first, std::string("pmStubCUDA::CopyDataToPinnedBuffers3"));
                        }
                    }
                }
            }
    ...
    if(lVector.size() > lAddressSpaceCount)
    {
        ...
        if(lCpuScratchBuffer && lScratchBufferSize)
            PMLIB_MEMCPY(lVector.back().pinnedPtr, lCpuScratchBuffer, lScratchBufferSize, std::string("pmStubCUDA::CopyDataToPinnedBuffers4"));
    }
...
pmStatus pmStubCUDA::CopyDataFromPinnedBuffers(pmTask* pTask, ulong pSubtaskId, pmSplitInfo* pSplitInfo, const pmSubtaskInfo& pSubtaskInfo)
{
    ...
    if(lVector.size() > lAddressSpaceCount)
    {
        ...
        if(lCpuScratchBuffer && lScratchBufferSize)
            PMLIB_MEMCPY(lCpuScratchBuffer, lVector.back().pinnedPtr, lScratchBufferSize, std::string("pmStubCUDA::CopyDataFromPinnedBuffers4"));
    }
...
{
    mCudaCacheAllocationLength += pLength;
    ++mCudaCacheAllocationCount;
}

void pmStubCUDA::RecordCudaCacheEviction(size_t pLength)
{
    mCudaCacheEvictionLength += pLength;
    ++mCudaCacheEvictionCount;
}
#endif
